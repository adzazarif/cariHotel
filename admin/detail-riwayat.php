            ...
            <tbody>
                <tr>
                    <?php
                        ...
                        $riwayat = mysqli_query($conn, "SELECT * FROM reservasi LEFT JOIN pelanggan USING (id_pelanggan) LEFT JOIN hotel USING (id) WHERE id_pelanggan = '".$_GET['id_pelanggan']."' $where ORDER BY id_reservasi DESC ");
                        if(mysqli_num_rows($riwayat)){
                            while($a = mysqli_fetch_array($riwayat)){
